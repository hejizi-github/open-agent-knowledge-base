...
| License | MIT（核心代码），enterprise/ 目录 source-available | README [ref: raw:README-content] |
...
| 层级 | 形态 | 技术栈 | 说明 |
|------|------|--------|------|
| L1 SDK | software-agent-sdk（独立仓库） | Python | 可组合式 Agent 库，支持本地到云端千级 Agent 扩展 [ref: raw:README-content] |
| L2 CLI | OpenHands-CLI（已拆分） | Python | 类 Claude Code / Codex 的终端交互体验 [ref: raw:README-content] |
| L3 GUI | Local GUI（本仓库） | React + FastAPI | 单页应用，类 Devin / Jules 体验 [ref: raw:README-content] |
| L4 Cloud | app.all-hands.dev | SaaS | 免费试用（Minimax 模型），含 Slack/Jira/Linear 集成 [ref: raw:README-content] |
| L5 Enterprise | 自托管 Kubernetes | Source-available | VPC 内部署，RBAC、协作、扩展支持 [ref: raw:README-content] |
...
实际实现通过 EventStream 消息传递完成 [ref: raw:openhands-README]：
...
1. **EventStream 事件驱动架构**：所有组件通过统一事件总线通信，Agent、Runtime、Frontend 松耦合 [ref: raw:openhands-README]
2. **五层产品矩阵**：从 SDK 到 Enterprise 的完整商业路径，同类项目中产品化程度最高
3. **Runtime 可插拔**：支持 Docker Sandbox 和本地运行（`RUNTIME=local`），Dev/Prod 一致 [ref: raw:AGENTS.md]
4. **多模型原生支持**：通过 LiteLLM 支持 Claude、GPT、Gemini、Qwen、Kimi、GLM 等主流模型 [ref: raw:openhands-releases-api.json]
5. **软件工程实践**：pre-commit、pytest、vitest、Mypy、Ruff 完整工具链；`.pr/` 目录机制管理临时产物 [ref: raw:AGENTS.md]
...
- raw:.evolve/raw/AGENTS.md — Repository contributor guide (https://raw.githubusercontent.com/OpenHands/OpenHands/main/AGENTS.md)
- raw:README-content — Project README (https://raw.githubusercontent.com/OpenHands/OpenHands/main/README.md)
- raw:openhands-README — Core module README (https://raw.githubusercontent.com/OpenHands/OpenHands/main/openhands/README.md)
- raw:pyproject.toml-content — Dependency and build config (https://raw.githubusercontent.com/OpenHands/OpenHands/main/pyproject.toml)
